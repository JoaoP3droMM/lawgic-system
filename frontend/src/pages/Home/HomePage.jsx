...
export default function HomePage() {
  const [notifications, setNotifications] = useState([]) // Começamos com dados de exemplo
  const [selectedFilter, setSelectedFilter] = useState(null)
  const [isLoading, setIsLoading] = useState(true) // false para ver os dados de exemplo
  ...
  const renderTableContent = () => {
    if (isLoading) {
      return (
        <tr>
          <td colSpan={4} className="message-cell">Carregando...</td>
        </tr>
      )
    }
    ...
    if (error) {
      return (
        <tr>
          <td colSpan={4} className="message-cell error-cell">{error}</td>
        </tr>
      )
    }
    ...
    if (notifications.length === 0) {
      return (
        <tr>
          <td colSpan={4} className="message-cell">Nenhuma notificação encontrada.</td>
        </tr>
      )
    }
